...
| D-014 | 2026-09-16 | Proposal branch `proposal/ayurveda-oncology-support` adds an additive `supportive_care` module (Ayurveda oncology evidence cards + Indian nutrition templates), gated by dual/triple clinical approval (oncology clinician/pharmacist + Ayurveda clinician, plus oncology dietitian for diet content) with no autonomous "recommended" state. See `docs/proposals/ayurveda-oncology-support/`. | Proposal brief; product/merge confirmation pending from Jonathan Zouari, distinct from clinical approval | proposed, not merged |
