...
| D-014 | 2026-09-17 | Proposal branch `proposal/chinese-herbal-oncology-support` adds an additive `supportive_care_evidence` module (Chinese herbal/TCM oncology evidence cards + symptom-based, non-cancer-specific nutrition templates + a staff-only alternative-modality discovery screen), gated by a quadruple approval model (oncology/pharmacy, evidence-methodology, and TCM-practitioner for herb content; oncology-dietitian for nutrition; no role substitutes for another) with no autonomous "recommended" state. Independent of the separate, still-unmerged `proposal/ayurveda-oncology-support` and `proposal/homeopathy-oncology-evidence`. See `docs/proposals/chinese-herbal-oncology-support/`. | Proposal brief; product/merge confirmation pending from Jonathan Zouari, distinct from clinical/evidence-methodology/dietetic approval | proposed, not merged |
