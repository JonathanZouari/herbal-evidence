...
| D-014 | 2026-09-17 | Proposal branch `proposal/homeopathy-oncology-evidence` adds an additive `supportive_care_evidence` module (homeopathy oncology evidence cards only, explicitly no nutrition content), gated by dual clinical/evidence-methodology approval with no autonomous "recommended" state and structural `blocked_integrity`/`blocked_no_results` states for the retracted NSCLC paper and withdrawn Banerji trial. Independent of the separate, still-unmerged `proposal/ayurveda-oncology-support`. See `docs/proposals/homeopathy-oncology-evidence/`. | Proposal brief; product/merge confirmation pending from Jonathan Zouari, distinct from clinical/evidence-methodology approval | proposed, not merged |
